# 기하학적 변환(Geometric Transform) : 이미지를 인위적으로 확대, 축소, 위치 변경, 회전, 왜곡하는 등 이미지의 형태를 변환하는 것을 의미함.
# 아핀 변환(Affine Transformation) : 2X3 행렬을 사용하며, 행렬 곱셈에 벡타 합을 활용해 표현할 수 있는 변환을 의미.
...
# 원근 변환 : 매핑에 사용할 변환 전 원본 이미지의 픽셀 좌표(srcPoint)과 변환 후 결과 이미지의 픽셀 좌표(dstPoint) 선언.
