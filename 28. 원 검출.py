...
# 최소거리 : 일차적으로 검출된 원과 원 사이의 최소 거리
# 캐니 엣지 임곗값 : 허프 변환에서 자체적으로 캐니 엣지를 적용하게 되는데, 이때 사용되는 상위 임곗값
# 중심 임곗값 : 그레이디언트 방법에 적용된 중심 히스토그램에 대한 임곗값이다. 이 값이 낮을수록 더 많은 원이 검출됨.
circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 100, param1=250, param2=10, minRadius=80, maxRadius=120)

# 검출을 통해 반환되는 circles 변수는 (1, N, 3)차원 형태를 갖는다.
# 반복문을 통해 circles 배열에서 중심점과 반지름을 반환할 수 있다.
for i in circles[0]:
