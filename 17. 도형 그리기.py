...
# 입력 이미지(src)에 좌측 상단 모서리 좌표(pt1)부터 우측 하단 모서리 좌표(pt2)로 구성된 특정 색상(color), 두께(thickness)의 사각형을 그림.
...
# 닫힘여부(isClosed)를 설정해 처음 좌표와 마지막 좌표의 연결 여부를 설정하며, 설정한 색상(color)과 두께(thickness)의 다각형이 그려짐.
