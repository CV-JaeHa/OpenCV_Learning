...
: 입력이미지(src)에 출력 이미지 정밀도(ddepth)를 설정하고 커널 크기(ksize), 비율(scale), 오프셋(delta), 
테두리 외삼법(borderType)을 설정하여 결과 이미지(dst) 반환."""
laplacian = cv2.Laplacian(gray, cv2.CV_8U, ksize=3)
"""캐니 함수 : 입력 이미지에서 가장자리를 검출 할 수 있다.
cv2.Canny(src, threshold1, threshold2, apertureSize, L2gradient)
: 입력 이미지(src)를 하위 임곗값(threshold1), 상위 임계값(threshold2), 
