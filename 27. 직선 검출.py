...
# 멀티 스케일 허프 변환(Multi-Scale Hough Transform) : 검출한 직선의 값이 더 정확한 값으로 반환되도록, 거리(p)와 각도(θ)의 값을 조정해 사용함.
